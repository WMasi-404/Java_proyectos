...
public abstract class AbsSet implements TADs.OperacionesConj{
	...
	public boolean pertenece(Object objElemento){
		...
		if (!estaVacio()){
			...
			while (temp!=null && !flag){
				if (!iguales(objElemento,temp.getNodoDato())){
	...
	public void sacar(Object objElemento){
		...
		if (!estaVacio()){
			if (pertenece(objElemento)){
				temp=this.conjunto;
				if (iguales(objElemento,temp.getNodoDato())){
					this.conjunto=this.conjunto.getNodoNext();
				}else{
					flag=false;
					while (temp.getNodoNext()!=null && !flag){
						if (!iguales(objElemento,temp.getNodoNext().getNodoDato())){
							temp=temp.getNodoNext();
						}else{
							flag=true;
						}
					}
					temp.setNodoNext(temp.getNodoNext().getNodoNext());					
				}
				this.cardinal--;
	...
	private Object sacarPrimero(){
		...
		if (!estaVacio()){
			objElemento=this.conjunto.getNodoDato();
			this.conjunto=this.conjunto.getNodoNext();
